...
export default async function init(nameArg?: string, options?: Options) {
	...
	if (import.meta.url.startsWith("file://")) {
		const src = `examples/${
			withUnocss ? "with-unocss/" : ""
		}${template}-app/`;
		await copyDir(src, name);
		alephPkgUri = "..";
	} else {
		console.log(
			`${dim("↓")} Downloading template(${blue(
				template!
		...
		const prefix = `${basename(repo)}-${VERSION}/examples/${
			withUnocss ? "with-unocss/" : ""
		}${template}-app/`;
	...
	const denoConfig = {
		...
		tasks: {
			dev: (await existsFile(join(appDir, "dev.ts")))
				? "deno run -A dev.ts"
				: `deno run -A ${alephPkgUri}/dev.ts`,
	...
	console.log(
		[
			...
			"",
			`${dim("$")} cd ${name}`,
			`${dim("$")} deno task dev    ${dim(
				"# Start the server in `development` mode"
			)}`,
			`${dim("$")} deno task start  ${dim(
				"# Start the server in `production` mode"
			)}`,
			`${dim("$")} deno task build  ${dim(
