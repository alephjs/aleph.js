...
export class Project {
  ...
  constructor(
    ...
    this.mode = mode;
    this.config = {
      srcDir: existsDirSync(path.join(this.appRoot, "/src/pages"))
        ? "/src"
        : "/",
      outputDir: "/dist",
      baseUrl: "/",
  ...
  private async _loadConfig() {
    ...
    if (util.isNEString(baseUrl)) {
      Object.assign(
        this.config,
        { baseUrl: util.cleanPath(encodeURI(baseUrl)) },
      );
    }
    ...
    if (typeof ssr === "boolean") {
      ...
      Object.assign(
        this.config,
        { ssr: { fallback, include, exclude, staticPaths } },
      );
    }
  ...
  private _moduleFromURL(url: string): Module {
    const isRemote = reHttp.test(url) ||
      (url in this.importMap.imports &&
        reHttp.test(this.importMap.imports[url]));
