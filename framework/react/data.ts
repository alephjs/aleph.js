...
export const useData = <T = any>(path?: string): DataState<T> & { mutation: typeof mutation } => {
  ...
  const action = useCallback(async (method: HttpMethod, fetcher: Promise<Response>, replace?: boolean) => {
    setDataStore(({ data }) => ({ data, isMutating: method }))
    const res = await fetcher
    const { status } = res
    if (status >= 400) {
      const message = await res.text()
      setDataStore(({ data }) => ({ data, error: { method, status, message } }))
      return res
    }
  ...
  const mutation = useMemo(() => {
    return {
      ...
      delete: async (params?: Record<string, string>, replace?: boolean) => {
        ...
        return action("delete", fetch(url.toString(), { method: "delete" }), replace)
      },
    }
  }, [path, routeUrl])
