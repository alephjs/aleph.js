...
export const serverStyles: Map<string, { css: string; asLink: boolean }> =
  new Map();
...
export default function Head(props: PropsWithChildren<{}>) {
  ...
  if (window.Deno) {
    parse(props.children).forEach(({ type, props }, key) =>
      renderer.cache.headElements.set(key, { type, props })
    );
  }
  ...
  return createElement(
    ...
    keywords &&
      createElement(
        ...
        {
          name: "keywords",
          content: util.isArray(keywords) ? keywords.join(",") : keywords,
        },
      ),
    ...
    twitter?.site &&
      createElement("meta", { name: "twitter:site", content: twitter.site }),
    twitter?.creator &&
      createElement(
        "meta",
...
export function applyCSS(id: string, css: string, asLink: boolean = false) {
  if (window.Deno) {
    ...
    const prevStyleEls = Array.from(document.head.children).filter((el: any) =>
      el.getAttribute("data-module-id") === id
    );
    if (asLink) {
...
function parse(
  node: ReactNode,
  els: Map<string, { type: string; props: Record<string, any> }> = new Map(),
) {
      ...
      case "no-script":
        {
          ...
          if (type === "meta") {
            ...
            if (propKeys.includes("name")) {
              ...
              key += Object.keys(props).filter((k) =>
                !(/^content|children$/i.test(k))
              ).map((k) => `[${k.toLowerCase()}=${JSON.stringify(props[k])}]`)
                .join("");
            }
          } else if (type !== "title") {
            key += "-" + (els.size + 1);
