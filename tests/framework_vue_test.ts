...
import VueSFCLoader from "../framework/vue/sfc-loader.ts";

Deno.test("[unit] vue/SFCLoader", async (t) => {
  await t.step("VueSFCLoader", async () => {
    const testVue = `
    <script setup>
      import { ref } from "https://esm.sh/v112/vue@3"
  ...
  `;
    ...
    assertStringIncludes(code, `createElementBlock as _createElementBlock } from "https://esm.sh/v112/vue"`);
  ...
  await t.step("VueSFCLoader(ssr)", async () => {
    ...
    assertStringIncludes(code, `ssrInterpolate as _ssrInterpolate } from "https://esm.sh/v112/@vue/server-renderer"`);
