...
export function compile(
  fileName: string,
  source: string,
  { mode, target: targetName, rewriteImportPath, reactRefresh, signUseDeno }:
    CompileOptions,
) {
  ...
  transformers.before!.push(
    createPlainTransformer(transformReactJsx, { mode, rewriteImportPath }),
  );
  transformers.after!.push(
    createPlainTransformer(
      transformReactUseDenoHook,
      { index: 0, signUseDeno },
    ),
  );
  transformers.after!.push(
    createPlainTransformer(transformImportPathRewrite, rewriteImportPath),
  );
