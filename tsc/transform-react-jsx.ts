...
export default function transformReactJsx(
  sf: ts.SourceFile,
  node: ts.Node,
  options: {
  ...
  if (ts.isJsxOpeningElement(node) || ts.isJsxSelfClosingElement(node)) {
    ...
    if (node.tagName.getText() === "Import") {
      for (let i = 0; i < props.length; i++) {
        const prop = props[i];
        if (
          ts.isJsxAttribute(prop) && prop.name.text === "from" &&
            ...
            ts.createJsxAttribute(
              ...
              ts.createJsxExpression(
                ...
                ts.factory.createStringLiteral(
                  path.join(path.dirname(sf.fileName), prop.initializer.text),
                ),
              ),
            ),
            ts.createJsxAttribute(
    ...
    if (options.mode === "development") {
      ...
      const lineNumberAttr = ts.createPropertyAssignment(
        ...
        ts.createNumericLiteral(
          (sf.getLineAndCharacterOfPosition(node.pos).line + 1).toString(),
        ),
      );
