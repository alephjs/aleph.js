...
export default function Link(props: PropsWithChildren<LinkProps>) {
  ...
  const prefetch = useCallback(() => {
    if (
      !util.isHttpUrl(href) && href !== currentHref &&
      !prefetchedPageModules.has(href)
    ) {
  ...
  if (Children.count(children) === 1) {
    ...
    if (isValidElement(child) && child.type === "a") {
      ...
      return cloneElement(child, {
        ...props,
        className:
          [className, props.className].filter(util.isNEString).join(" ") ||
          undefined,
  ...
  if (currentPathname === pathname) {
    return React.createElement(
      Link,
      {
        ...rest,
        to,
        className: [rest.className?.trim(), activeClassName.trim()].filter(
          Boolean,
        ).join(" "),
