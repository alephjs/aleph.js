...
export function getAppDir() {
	return globalItSync("__ALEPH_APP_DIR", () =>
		Deno.mainModule
...
export async function watchFs(
	...
	const reIgnore =
		/[\/\\](\.git(hub)?|\.vscode|vendor|node_modules|dist|out(put)?|target)[\/\\]/;
	const ignore = (path: string) =>
		reIgnore.test(path) || path.endsWith(".DS_Store");
	const allFiles = new Set<string>(
		(await getFiles(rootDir))
			.map(name => join(rootDir, name))
			.filter(path => !ignore(path))
	);
	for await (const { kind, paths } of Deno.watchFs(rootDir, {
		recursive: true,
	})) {
