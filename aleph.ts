...
export function ALEPH({ initial }: {
  ...
  const [e404, setE404] = useState<
    { Component: ComponentType<any>; props?: Record<string, any> }
  >(() => {
    ...
    if (E404) {
      ...
      return {
        Component: E400MissingDefaultExportAsComponent,
        props: { name: "Custom 404 Page" },
      };
    }
    return { Component: E404Page };
  });
  const [app, setApp] = useState<
    { Component: ComponentType<any> | null; props?: Record<string, any> }
  >(() => {
  ...
  const onpopstate = useCallback(async (e: any) => {
    ...
    if (url.pagePath !== "") {
      const ctree: { id: string; Component?: ComponentType<any> }[] =
        pageModuleTree.map(({ id }) => ({ id }));
      const imports = pageModuleTree.map(async (mod) => {
        const { default: C } = await import(
          getModuleImportUrl(baseUrl, mod, e.forceRefetch)
        );
        if (mod.deps) {
          ...
          for (const dep of mod.deps.filter(({ isStyle }) => !!isStyle)) {
            await import(
              getModuleImportUrl(
                baseUrl,
                {
                  id: util.ensureExt(dep.url.replace(reHttp, "/-/"), ".js"),
                  hash: dep.hash,
                },
                e.forceRefetch,
              )
          ...
          if (
            mod.deps.filter(({ isData, url }) =>
              !!isData && url.startsWith("#useDeno.")
            ).length > 0
          ) {
            const { default: data } = await import(
              `/_aleph/data${
                [url.pathname, url.query.toString()].filter(Boolean).join("@")
              }/data.js` + (e.forceRefetch ? `?t=${Date.now()}` : "")
            );
            if (util.isPlainObject(data)) {
              for (const key in data) {
                const useDenoUrl =
                  `useDeno://${url.pathname}?${url.query.toString()}#${key}`;
      ...
      switch (mod.id) {
        case "/404.js": {
          const { default: Component } = await import(
            getModuleImportUrl(baseUrl, mod, true)
          );
  ...
  return (
    React.createElement(
      ...
      React.createElement(
        ...
        { value: route.url },
        ...[
          (route.Page && app.Component) &&
          React.createElement(
            app.Component,
            Object.assign(
              {},
              app.props,
              { Page: route.Page, pageProps: route.pageProps },
            ),
          ),
          (route.Page && !app.Component) &&
          React.createElement(route.Page, route.pageProps),
...
export function getModuleImportUrl(
  baseUrl: string,
  mod: RouteModule,
  forceRefetch = false,
) {
  return util.cleanPath(
    baseUrl + "/_aleph/" +
      (mod.id.startsWith("/-/") ? mod.id : util.trimSuffix(mod.id, ".js") +
        `.${mod.hash.slice(0, hashShort)}.js`) +
      (forceRefetch ? `?t=${Date.now()}` : ""),
  );
}
