...
export async function renderPage(
  ...
  rendererCache.scriptsElements.forEach(({ props }) => {
    ...
    if (
      dangerouslySetInnerHTML && util.isNEString(dangerouslySetInnerHTML.__html)
    ) {
  ...
  await Promise.all(
    styles?.map(({ url, hash }) => {
      const path = reHttp.test(url)
        ? url.replace(reHttp, "/-/")
        : `${url}.${hash.slice(0, hashShort)}`;
      return import(
        "file://" +
          util.cleanPath(
            `${Deno.cwd()}/.aleph/${buildMode}.${buildTarget}/${path}.js`,
          )
      );
    }) || [],
  );
  styles?.forEach(({ url }) => {
    if (serverStyles.has(url)) {
      ...
      if (asLink) {
        ret.head.push(
          `<link rel="stylesheet" href="${css}" data-module-id=${
            JSON.stringify(url)
          } />`,
        );
      } else {
        ret.head.push(
          `<style type="text/css" data-module-id=${
            JSON.stringify(url)
          }>${css}</style>`,
        );
      }
